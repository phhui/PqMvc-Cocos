// /**PqView为以前写的自定义UI解析器，可以将自己定义的组件通过JSON解析成UI界面，具体和XML类似
//  * 初始化时传入UI模块名称，名称必需为对应的配置资源名称，程序会根据名称从资源配置中读取相应的配置并解析渲染成UI界面
//  * 优点：可以自定义任何组件，任意界面，任意属性
//  * 缺点：没有UI编辑器，需要手动写JSON拼界面
//  * 有一个简单的编辑器，但仅自用，没完善，自己都不怎么用，用得少也就懒得去完善了**/
// class PqView extends cc.Component {
...
//         this.execute();
//     }
// 	protected addListen(event:string,func:Function,target:any){
// 		EventHelper.on(event,func,target);
// 	}
// 	protected call(event:string,...args){
// 		args.unshift(event);
// 		EventHelper.emit.apply(EventHelper,args);
// 	}
// 	protected removeListen(event:string,func:Function=null,target:any=null){
...
//         return ResUtils.dict[this.moduleName+"_"+name];
//     }
...
//         let t= ResUtils.dict[this.moduleName+"_"+key];
...
//             var obj: any;
//             if(v.type=="sound"&&v.url)ResUtils.dict[this.moduleName+"_"+v.id]=ResUtils.getSound(v.url);
//             else if(v.url&&v.type=="sheet")ResUtils.dict[this.moduleName+"_"+v.id]=ResUtils.dict[v.url];
//             else if(v.url)ResUtils.dict[this.moduleName+"_"+v.id]=ResUtils.getResByUrl(v.url).texture;
//             else if(v.resName)ResUtils.dict[v.id]=RES.getRes(v.resName);
...
//         sp.name = v.id;
//         if (v.url) {
//             var img: egret.Bitmap = (v.url?ResUtils.getResByUrl(v.url):ResUtils.getSheet(v.source,this.moduleName));
...
//         else if(v.url)img=ResUtils.getResByUrl(v.url);
//         else if(v.source)img=ResUtils.getSheet(v.source,this.moduleName);
...
//         var sp: egret.Sprite = new egret.Sprite();
//         if(v.url||v.source){
//             var bm: egret.Bitmap = (v.url?ResUtils.getResByUrl(v.url):ResUtils.getSheet(v.source,this.moduleName));
