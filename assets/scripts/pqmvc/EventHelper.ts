class EventHelper{
    ...
    static on(key:string,func:Function,target:any){
        this._listener(key,func,target);
    }
    static _listener(key:string,func:Function,target:any,once:boolean=false){
        if(EventHelper.checkRepeat(key,func,target)){
            // console.log(key+"--重复监听！");
            return;
        }
    ...
    static remove(key:string,func:Function=null,target:any=null){
        if(this.listenList[key]){
            if(target){
                ...
                throw new Error("消息【"+key+"】存在多个监听，请指定事件函数进行删除");
            }else this.listenList[key]=null;
        }
    }
    static emit(key:string,...args){
        args.unshift(key);
        if(this.listenList[key]){
            ...
            //console.log("系统中没有针对"+key+"消息的监听！");
            //throw new Error("系统中没有针对"+key+"消息的监听！");
        } 
    }
